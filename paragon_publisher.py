...
class HarvesterDialog(ctk.CTkToplevel):
    ...
    def _save_config(self):
        try:
            ...
            cfg["harvester_min_len"] = self.min_len_entry.get().strip()
            cfg["harvester_max_len"] = self.max_len_entry.get().strip()
    ...
    def _create_ui(self, cfg):
        ...
        ParagonLabel(opt, text="Length", style="muted", anchor="w").pack(side="left")
        self.min_len_entry = ParagonEntry(opt, width=42)
        self.min_len_entry.insert(0, str(cfg.get("harvester_min_len", "")))
        self.min_len_entry.pack(side="left", padx=(6, 2))
        ParagonLabel(opt, text="–", style="muted").pack(side="left")
        self.max_len_entry = ParagonEntry(opt, width=42)
        self.max_len_entry.insert(0, str(cfg.get("harvester_max_len", "")))
    ...
    def _start_download(self, organize, urls=None, force_archive=False):
        ...
        def _len_secs(entry):
            txt = entry.get().strip()
            try:
                return int(float(txt) * 60) if txt else None
            except ValueError:
                return None
        min_duration = _len_secs(self.min_len_entry)
        max_duration = _len_secs(self.max_len_entry)
    ...
    def _fix_genre(self):
        """Set the genre (from the Default genre field) on every processed file
        under a chosen folder -- filenames, episode NFOs and tvshow.nfo. Same
        folder-pick flow as Fix Titles; point it at one show folder or a parent.
        """
        if self._worker and self._worker.is_alive():
            return
        ...
        if not new_genre:
            messagebox.showwarning(
                "Fix Genre",
                "Type the new genre in the 'Default genre' field first, then pick a folder.",
                parent=self)
            return
        start = self.dest_entry.get().strip() or str(Path.home())
        folder = filedialog.askdirectory(
            title=f"Select folder to set genre to '{new_genre}' (recurses into show folders)",
            initialdir=start, parent=self)
        if not folder:
            return
        self._busy(True, monitoring=False)
        ...
        def work():
            errored = False
            try:
                changes = paragon_harvester.retitle_genre_in_folder(
                    folder, new_genre, apply=False)
                self.after(0, lambda: self._fix_genre_confirm(folder, new_genre, changes))
            except Exception as e:
                errored = True
